...
public class WheelOptions<T> {
    ...
    public void setPicker(List<T> options1Items, List<List<T>> options2Items, List<List<List<T>>> options3Items) {
        ...
        // 选项1
        wv_option1.setAdapter(new ArrayWheelAdapter<T>(mOptions1Items)); // 设置显示数据
    ...
    public void setNPicker(List<T> options1Items, List<T> options2Items, List<T> options3Items) {
        ...
        // 选项1
        wv_option1.setAdapter(new ArrayWheelAdapter<T>(options1Items)); // 设置显示数据
        wv_option1.setCurrentItem(0); // 初始化时显示的数据
        // 选项2
        if (options2Items != null) {
            wv_option2.setAdapter(new ArrayWheelAdapter<T>(options2Items)); // 设置显示数据
        }
        wv_option2.setCurrentItem(wv_option2.getCurrentItem()); // 初始化时显示的数据
        // 选项3
        if (options3Items != null) {
            wv_option3.setAdapter(new ArrayWheelAdapter<T>(options3Items)); // 设置显示数据
        }
        wv_option3.setCurrentItem(wv_option3.getCurrentItem());
        wv_option1.setIsOptions(true);
        wv_option2.setIsOptions(true);
    ...
    private void itemSelected(int opt1Select, int opt2Select, int opt3Select) {
        ...
        if (mOptions2Items != null) {
            wv_option2.setAdapter(new ArrayWheelAdapter<T>(mOptions2Items.get(opt1Select)));
            wv_option2.setCurrentItem(opt2Select);
        }
        if (mOptions3Items != null) {
            wv_option3.setAdapter(new ArrayWheelAdapter<T>(mOptions3Items.get(opt1Select).get(opt2Select)));
